///
/// HybridWebImageLoaderSpec.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2025 Marc Rousavy @ Margelo
///

#pragma once

#include <NitroModules/JHybridObject.hpp>
#include <fbjni/fbjni.h>
#include "HybridWebImageLoaderSpec.hpp"

// Forward declaration of `JHybridImageLoaderSpec` to properly resolve imports.
namespace margelo::nitro::web::image { class JHybridImageLoaderSpec; }
#include "JHybridImageLoaderSpec.hpp"

namespace margelo::nitro::web::image {

  using namespace facebook;

  class JHybridWebImageLoaderSpec: public jni::HybridClass<JHybridWebImageLoaderSpec, JHybridImageLoaderSpec>,
                                   public virtual HybridWebImageLoaderSpec {
  public:
    static auto constexpr kJavaDescriptor = "Lcom/margelo/nitro/web/image/HybridWebImageLoaderSpec;";
    static jni::local_ref<jhybriddata> initHybrid(jni::alias_ref<jhybridobject> jThis);
    static void registerNatives();

  protected:
    // C++ constructor (called from Java via `initHybrid()`)
    explicit JHybridWebImageLoaderSpec(jni::alias_ref<jhybridobject> jThis) :
      HybridObject(HybridWebImageLoaderSpec::TAG),
      HybridBase(jThis),
      _javaPart(jni::make_global(jThis)) {}

  public:
    ~JHybridWebImageLoaderSpec() override {
      // Hermes GC can destroy JS objects on a non-JNI Thread.
      jni::ThreadScope::WithClassLoader([&] { _javaPart.reset(); });
    }

  public:
    size_t getExternalMemorySize() noexcept override;

  public:
    inline const jni::global_ref<JHybridWebImageLoaderSpec::javaobject>& getJavaPart() const noexcept {
      return _javaPart;
    }

  public:
    // Properties
    

  public:
    // Methods
    std::shared_ptr<Promise<std::shared_ptr<margelo::nitro::web::image::HybridImageSpec>>> loadImage() override;
    void requestImage(const std::shared_ptr<margelo::nitro::web::image::HybridNitroImageViewSpec>& forView) override;
    void dropImage(const std::shared_ptr<margelo::nitro::web::image::HybridNitroImageViewSpec>& forView) override;

  private:
    friend HybridBase;
    using HybridBase::HybridBase;
    jni::global_ref<JHybridWebImageLoaderSpec::javaobject> _javaPart;
  };

} // namespace margelo::nitro::web::image
