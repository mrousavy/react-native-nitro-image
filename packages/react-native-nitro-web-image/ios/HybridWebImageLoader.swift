...
class HybridWebImageLoader: HybridImageLoaderSpec {
    ...
    func requestImage(forView view: (any HybridNitroImageViewSpec)) throws {
        guard let view = view as? NativeImageView else {
            throw RuntimeError.error(withMessage: "Invalid view type!")
        }

        let webImageOptions = options?.toSDWebImageOptions() ?? []
        let webImageContext = options?.toSDWebImageContext()
        view.imageView.sd_setImage(
            with: url,
    ...
    public static func loadImage(url: URL, options: AsyncImageLoadOptions?) -> Promise<
        any HybridImageSpec
    > {
        return Promise.async {
            let uiImage = try await SDWebImageManager.shared.loadImage(
                with: url,
                options: options)
