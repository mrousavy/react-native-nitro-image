...
internal final class ImageCache: ImageCaching {
    ...
    /// By default, `0.1`.
    internal var entryCostLimit: Double {
        get { impl.conf.entryCostLimit }
        set { impl.conf.entryCostLimit = newValue }
    }

    /// The total number of items in the cache.
    internal var totalCount: Int { impl.totalCount }

    /// The total cost of items in the cache.
    internal var totalCost: Int { impl.totalCost }
    ...
    internal init(costLimit: Int = ImageCache.defaultCostLimit(), countLimit: Int = Int.max) {
    ...
    internal static func defaultCostLimit() -> Int {
    ...
    internal subscript(key: ImageCacheKey) -> ImageContainer? {
